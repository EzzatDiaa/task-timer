    ...
    const fetchTask = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          countdownDuration: taskData.countdownDuration,
          alarmTime: taskData.alarmTime,
        });

        // Initialize time inputs
        if (
          taskData.timerType === TimerType.COUNTDOWN &&
          taskData.countdownDuration
        ) {
          const totalSeconds = taskData.countdownDuration;
          setHours(Math.floor(totalSeconds / 3600));
          setMinutes(Math.floor((totalSeconds % 3600) / 60));
          setSeconds(totalSeconds % 60);
        } else if (
          taskData.timerType === TimerType.ALARM &&
          taskData.alarmTime
        ) {
          const date = new Date(taskData.alarmTime);
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              disabled={task.timerStatus !== "idle"} // Only allow changing timer type when idle
            >
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    disabled={task.timerStatus !== "idle"} // Only allow changes when idle
                  />
                </div>
                <div>
                  <label
